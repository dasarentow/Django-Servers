...
class CommentSerializer (ModelSerializer):
    ...
    class Meta:
        ...
        fields = "__all__"
        read_only_fields = (
            'host', 'created', 'updated', 'slug', 'is_children', 'is_parent', 'response', 'see'
        )
...
class TopicSerializer(WritableNestedModelSerializer):
    link = serializers.HyperlinkedIdentityField(
        view_name='chat:topic-detail', lookup_field='slug', )
    created_by = UserProfileSerializer(read_only=True, many=False)
    comment = CommentSerializer(read_only=True, many=True)
    response = ResponseSerializer(read_only=True, many=True)
    like = LikeSerializer(read_only=True, many=True)
    load_pic = serializers.ImageField(required=False, )

    class Meta:
        model = Topic
        fields = ['id', 'topic', 'description', 'load_pic',
                  'created', 'updated', 'link', 'created_by', 'comment', 'response', 'likes', 'like', 'slug']
        read_only_fields = ('created', 'updated', 'slug',
                            'sow', 'comment', 'response', )
