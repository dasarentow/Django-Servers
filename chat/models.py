...
class Field(models.Model):
    ...
    category = models.CharField(
        max_length=200, choices=SUBJECT_CATEGORY, default=any)
    topic = models.ForeignKey(
        Topic, on_delete=models.CASCADE, related_name='topics')
    updated = models.DateTimeField(auto_now=True)
    created = models.DateTimeField(auto_now_add=True)
    slug = AutoSlugField(
        populate_from=['category', "topic",])
...
class Comments(models.Model):
    ...
    topic = models.ForeignKey(
        Topic, on_delete=models.CASCADE, related_name='comment')
...
class Responses(models.Model):
    host = models.ForeignKey(User, related_name='hosts',
                             null=True, blank=True, on_delete=models.CASCADE)
    comment = models.ForeignKey(
        Comments, on_delete=models.CASCADE, related_name='reply')
    participants = models.ManyToManyField(
        User, related_name='participant')
...
class Like(models.Model):
    ...
    topic = models.ForeignKey(
        Topic, related_name='like', on_delete=models.CASCADE)
    participants = models.ManyToManyField(
        User, related_name='all_participants')
    likes = models.IntegerField(default=0)
